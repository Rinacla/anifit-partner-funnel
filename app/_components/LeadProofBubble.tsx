...
export default function LeadProofBubble() {
  ...
  return (
    <div className="fixed bottom-24 left-4 z-50 animate-fade-in-up md:bottom-8">
      <div className="bg-white/95 backdrop-blur-sm border border-green-100 rounded-2xl p-3 shadow-xl flex items-center gap-3 max-w-[280px]">
        ...
        <div>
          ...
          <p className="text-[9px] text-green-600 font-medium uppercase tracking-wider mt-1">
            VOR WENIGEN MINUTEN
          </p>
        </div>
        <button 
          onClick={() => setVisible(false)}
          className="absolute -top-1 -right-1 bg-white border border-gray-100 text-gray-400 hover:text-gray-600 rounded-full w-5 h-5 flex items-center justify-center text-[10px] shadow-sm"
        >
          ✕
        </button>
      </div>
